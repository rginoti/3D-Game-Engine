...
//	float directionalLightSpec = pow(max(dot(viewDir, directionalLightReflectDir), 0.0), material.shininess);
	...
	float attenuation1 = 1.0 / (pointLight.constant + pointLight.linear * pointLightDistance + pointLight.quadratic * (pointLightDistance * pointLightDistance));
	float attenuation2 = 1.0 / (spotLight.constant + spotLight.linear * spotLightDistance + spotLight.quadratic * (spotLightDistance * spotLightDistance));
	...
	spotLightDiffuse *= intensity;
	spotLightSpecular *= intensity;
	
	pointLightDiffuse *= attenuation1;
	pointLightSpecular *= attenuation1;
	spotLightDiffuse *= attenuation2;
	spotLightSpecular *= attenuation2;
